...
stat /etc/crontab | grep "Изменён: " | awk -F "Изменён: " '{print $2}' | sed "s/ //g" | tee $LOG > /dev/null
fi
cron_stat=`stat /etc/crontab | grep "Изменён: " | awk -F "Изменён: " '{print $2}' | sed "s/ //g"`
